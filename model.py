...
from typing import Dict, TypedDict, List, Optional, Literal, Annotated
...
def reducer_node(state: State) -> dict:
    ...
    # Use approved_sections if available, otherwise use sections
    sections_to_use = state.get("approved_sections") or state.get("sections", [])
    ordered_sections = [md for _, md in sorted(sections_to_use, key=lambda x: x[0])]
...
# # 11) Runner
# # -----------------------------
# # def run(topic: str, config: Optional[Dict] = None, as_of: Optional[str] = None):
